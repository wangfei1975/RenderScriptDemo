 ...
 */
public final class RsNV12ImageProcessor {
    ...
    final ScriptC_NV12ImageProcessor mScript;
    ...
    public static final class Builder {
        ...
        public RsNV12ImageProcessor build() {
            checkParams(sw, sh,cl, ct, cr, cb, dw, dh, rot, mir);
            return new RsNV12ImageProcessor(rs, sw, sh, cl, ct, cr, cb, dw, dh, rot, mir);
        }
    }
    ...
    *
    * */
    private RsNV12ImageProcessor(final RenderScript rs,
                                final int srcWidth,
                                final int srcHeight,
                                final int cropLeft,
                                final int cropTop,
        ...
        mRS = rs;
        mScript = new ScriptC_NV12ImageProcessor(mRS);
        mRotation = rotation;
        mMirror = mirror;
        mSrcWidth = srcWidth;
        ...
        mScript.invoke_prepare_processor(srcWidth, srcHeight, cropLeft, cropTop, cropRight, cropBottom,
                dstWidth, dstHeight);
        ...
        Type typeDst = new Type.Builder(mRS, Element.U8(mRS)).setX(mDstWidth).setY(mDstHeight * 3 / 2).create();
        mOut = Allocation.createTyped(mRS, typeDst, Allocation.USAGE_SCRIPT);
        mOption = new Script.LaunchOptions().setX(0, mDstWidth).setY(0, mDstHeight);
        mUVOption = new Script.LaunchOptions().setX(0, mDstWidth).setY(mDstHeight, mDstHeight * 3 / 2);
    }
    ...
    NV12Image process(final NV12Image in) {
       return process(new NV12Image(mDstWidth, mDstHeight), in);
    }
